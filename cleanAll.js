...
function replaceMisc() {
  ...
  eesRepo = eesRepo.replace(/(?m)^\s*adRelated:.*$\n?/g, '');
  ...
  pdfSvc = pdfSvc.replace(/(?m)^\s*adRelated: item\.adRelated \|\| '-',$\n?/g, '');
  pdfSvc = pdfSvc.replace(/(?m)^\s*html \+= <td rowspan="\$\{item\.groupLength\}">\$\{item\.adRelated\}<\/td>;$\n?/g, '');
